...
fold = sys.argv[2]
data = sys.argv[3]
save_path = sys.argv[4] if len(sys.argv) > 4 else "/scratch/project_2009199/umap-embeddings/long_model_embeds/xlmr-fold-"+str(fold)+"/"

base_model_name ="xlm-roberta-base"
model_name = "/scratch/project_2009199/pytorch-registerlabeling/models/xlm-roberta-large_testing_longer_run/labels_upper/en-fi-fr-sv-tr_en-fi-fr-sv-tr/seed_42/fold_"+str(fold)
device = "cuda:0" if torch.cuda.is_available() else "cpu"

dataset = datasets.load_from_disk(data+str(lang)+".hf")
print(dataset)
#dataset = dataset.filter(lambda example, idx: idx % 10 == 1, with_indices=True)
...
def predict(d,lang):
    ...
    #print(sigm)
    pred = [labels[i] for i in np.where(sigm > 0.5)[0]]
    #print(pred)
