...
lang = sys.argv[1]
data = sys.argv[2]
save_path = sys.argv[3] if len(sys.argv) > 3 else "/scratch/project_2009199/umap-embeddings/model_embeds/bge-m3/"
...
def predict(d,lang):
    ...
    #print(sigm)
    #print(np.where(sigm >0.5)[0])
    #print([i for i in np.where(sigm >0.5)[0]])
    pred = [labels[i] for i in np.where(sigm > 0.5)[0] if i < 9] # small labels (>=9) get the heck out
    #print(pred)
    hidden_states = output["hidden_states"]
